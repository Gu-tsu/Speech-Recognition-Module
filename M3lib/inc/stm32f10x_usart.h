...
typedef struct
{
  ...
  uint16_t USART_WordLength;	//发送/接收 的字长
  uint16_t USART_StopBits;		//停止位
  uint16_t USART_Parity;	   //奇偶校验
  uint16_t USART_Mode;		   //发送/接收 的模式
  uint16_t USART_HardwareFlowControl;  //硬件流控制模式使能还是失能 
} USART_InitTypeDef;
...
typedef struct
{
  uint16_t USART_Clock;	  //时钟使能还是失能
  uint16_t USART_CPOL;	  //定了下SLCK引脚上时钟输出的极
  uint16_t USART_CPHA;	   //定了下SLCK引脚上时钟输出的相位
  uint16_t USART_LastBit;	 //是否在同步模式下，在SCLK引脚上输出
  							//最后发送的那个数据字 (MSB)对应的时钟脉冲
